namespace MarioGame
{
    ...
    public class Camera
    {
        ...
        public bool InCamera(Vector2 position)
        {
            bool xCheck = (position.X >= Position.X && position.X < (Position.X + _viewport.Width));
            bool yCheck = (position.Y >= Position.Y && position.Y < (Position.Y + _viewport.Height));
            return xCheck && yCheck;
        }

        public bool BelowMap(Vector2 position)
        {
            bool yCheck = position.Y >= (Position.Y + ((Rectangle)_limits).Height + 20f);
            return yCheck;
        }
